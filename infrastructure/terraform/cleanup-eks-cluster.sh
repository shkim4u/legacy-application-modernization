...
kubectl patch targetgroupbindings `kubectl get targetgroupbindings -l ingress.k8s.aws/stack=kaili -n istio-system -o jsonpath='{.items[*].metadata.name}'` -n istio-system -p '{"metadata":{"finalizers":null}}' --type=merge
